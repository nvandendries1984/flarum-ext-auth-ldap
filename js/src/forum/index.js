...
const translationPrefix = 'nielsvandendries-auth-ldap.forum.';

app.initializers.add('nielsvandendries-auth-ldap', () => {
	...
	function overrideModal() {
		if (app.forum.attribute('nielsvandendries-auth-ldap.onlyUse')) {
	...
	function addLoginLink(items) {
		if (items.has('logIn')) {
          ...
          app.translator.trans(translationPrefix + 'log_in_with', {server: app.forum.attribute('nielsvandendries-auth-ldap.method_name')})
				),
        0
			);
		}
